...
class Report(db.Model):
    ...
    def target_name(self):
        ...
        if self.target_type.value in ('USER', 'COMPANION'):
            user = User.query.get(self.target_id)
            if user:
                cp = CustomerProfile.query.filter_by(user_id=user.user_id).first()
                comp = CompanionProfile.query.filter_by(user_id=user.user_id).first()
                if cp:
                    return cp.full_name or user.email
                if comp:
                    return comp.display_name or user.email
                return user.email
        elif self.target_type.value == 'BOOKING':
            return f"Booking #{self.target_id}"
